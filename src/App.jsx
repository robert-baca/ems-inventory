...
function MapView({ locations, library, stock, categories, mapData, navigate, onSaveMap }) {
  ...
  function undo() {
    ...
    setHistory(h => h.slice(0,-1)); setMap(prev); onSaveMap(prev);
  }
  function clearAll() {
    if (!window.confirm('Clear the entire map?')) return;
    const cleared = { rooms:[], pins:[], lines:[], doors:[], bgImage:null };
    setHistory([]); setMap(cleared); onSaveMap(cleared);
    setSelectedRoom(null); setSelectedPin(null); setSelectedLine(null); setSelectedDoor(null);
  }
  function addRoom() {
    const r = { id: uid(), name: 'New Room', x: 60, y: 60, w: 220, h: 160, color: '#e2e8f0' };
    updateMap({ ...map, rooms: [...(map.rooms||[]), r] });
    setSelectedRoom(r.id); setTool('select');
  }
  function addPin(locId) {
    const pins = map.pins || [];
    if (pins.find(p => p.locationId === locId)) return;
    updateMap({ ...map, pins: [...pins, { id: uid(), locationId: locId, x: 150, y: 150 }] });
  }
  function deleteSelected() {
    if (selectedRoom) { updateMap({ ...map, rooms: (map.rooms||[]).filter(r => r.id !== selectedRoom) }); setSelectedRoom(null); }
    if (selectedLine) { updateMap({ ...map, lines: (map.lines||[]).filter(l => l.id !== selectedLine) }); setSelectedLine(null); }
    if (selectedDoor) { updateMap({ ...map, doors: (map.doors||[]).filter(d => d.id !== selectedDoor) }); setSelectedDoor(null); }
  }
  ...
  function handleCanvasMouseUp() {
    if (drawingLine) {
      ...
      if (len > 10) updateMap({ ...map, lines: [...(map.lines||[]), { id: uid(), ...drawingLine, thickness: 6, color: '#334155' }] });
  ...
  function startDrag(e, type, id) {
    ...
    const item = type.startsWith('room') ? (map.rooms||[]).find(r=>r.id===id) : type==='pin' ? (map.pins||[]).find(p=>p.id===id) : (map.doors||[]).find(d=>d.id===id);
    setDragItem({ type, id, startMouseX: e.clientX, startMouseY: e.clientY, startX: item?.x||0, startY: item?.y||0, startW: item?.w||0, startH: item?.h||0 });
  }
  ...
  async function handleBgUpload(e) {
    ...
    reader.onload = async ev => { const resized = await resizeImage(ev.target.result.split(',')[1], 1400); updateMap({ ...map, bgImage: resized }); };
  ...
  selectedStock.forEach(s => {
    const item = library.find(i => i.id === s.libraryId); if (!item) return;
    if (!selectedGrouped[item.id]) selectedGrouped[item.id] = { item, entries: [] };
    selectedGrouped[item.id].entries.push(s);
  });
      ...
      <div style={{ background: 'var(--color-bg)', borderBottom: '1px solid var(--color-border)', padding: '8px 14px', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', flexShrink: 0 }}>
        ...
        {editing && (
          <>
            ...
            {[['select','↖ Select'],['line','📏 Line'],['door','🚪 Door']].map(([t,l]) => (
              <button key={t} onClick={() => setTool(t)} style={{ ...tool===t?btnP:btnS, padding:'7px 12px', fontSize:12 }}>{l}</button>
            ))}
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
        <div ref={canvasRef} style={{ flex:1, overflow:'auto', position:'relative', background:'var(--color-bg)', cursor: tool==='line'?'crosshair':tool==='door'?'copy':'default' }}
          ...
          <svg width={SVG_W} height={SVG_H} style={{ position:'absolute', top:0, left:0, pointerEvents:'none' }}>
            ...
            {(map.lines||[]).map(line => {
              ...
              return <g key={line.id}>
                <line x1={line.x1} y1={line.y1} x2={line.x2} y2={line.y2} stroke={isSel?'#1a1a1a':line.color||'#334155'} strokeWidth={line.thickness||6} strokeLinecap="round" style={{ cursor:editing?'pointer':'default', pointerEvents:'stroke' }} onClick={e=>{ if(editing){e.stopPropagation();setSelectedLine(line.id);setSelectedRoom(null);setSelectedDoor(null);} }} />
                {editing&&isSel&&<>
                  <circle cx={line.x1} cy={line.y1} r={8} fill="#1a1a1a" stroke="#fff" strokeWidth={2} style={{ cursor:'move', pointerEvents:'all' }} onMouseDown={e=>{e.stopPropagation();startLineDrag(e,'line-p1',line.id);}} />
                  <circle cx={line.x2} cy={line.y2} r={8} fill="#1a1a1a" stroke="#fff" strokeWidth={2} style={{ cursor:'move', pointerEvents:'all' }} onMouseDown={e=>{e.stopPropagation();startLineDrag(e,'line-p2',line.id);}} />
                </>}
              </g>;
            })}
            ...
            {(map.doors||[]).map(door => {
              const isSel = selectedDoor===door.id; const s = door.size||40;
              return <g key={door.id} transform={`translate(${door.x},${door.y}) rotate(${door.rotation||0})`} style={{ cursor:editing?'move':'default', pointerEvents:'all' }}
                onMouseDown={e=>{e.stopPropagation();startDrag(e,'door',door.id);setSelectedDoor(door.id);setSelectedRoom(null);setSelectedLine(null);}}
                onClick={e=>{if(editing){e.stopPropagation();setSelectedDoor(door.id);setSelectedRoom(null);setSelectedLine(null);}}}>
                <rect x={-2} y={-2} width={s+4} height={8} fill={isSel?'#1a1a1a':'#475569'} rx={2} />
                <path d={`M 0 6 Q ${s/2} 6 ${s} ${s+6}`} fill="none" stroke={isSel?'#1a1a1a':'#475569'} strokeWidth={2} strokeDasharray="4 3" />
                <line x1={0} y1={6} x2={s} y2={6} stroke={isSel?'#1a1a1a':'#475569'} strokeWidth={3} strokeLinecap="round" />
              </g>;
            })}
          </svg>
          <div style={{ position:'absolute', top:0, left:0, width:SVG_W, height:SVG_H }}>
            ...
            {(map.pins||[]).map(pin => {
              ...
              const isPanelOpen = selectedPin===loc.id;
              return <div key={pin.id} style={{ position:'absolute', left:pin.x, top:pin.y, userSelect:'none', zIndex:20 }}
                onMouseDown={e=>{if(editing){e.stopPropagation();startDrag(e,'pin',pin.id);}}}
        ...
        {editing&&selectedLine&&(
          <div style={{ width:220, background:'var(--color-bg)', borderLeft:'1px solid var(--color-border)', padding:16, flexShrink:0 }}>
            ...
            <div style={{ marginBottom:12 }}>
              <label style={{ display:'block', fontSize:12, color:'var(--color-text-secondary)', marginBottom:5 }}>Thickness</label>
              <input type="range" min={2} max={24} value={(map.lines||[]).find(l=>l.id===selectedLine)?.thickness||6} onChange={e=>{setMap(m=>({...m,lines:(m.lines||[]).map(l=>l.id===selectedLine?{...l,thickness:+e.target.value}:l)}));onSaveMap(map);}} style={{ width:'100%' }} />
            </div>
            <div style={{ marginBottom:12 }}>
        ...
        {editing&&selectedDoor&&(
          <div style={{ width:220, background:'var(--color-bg)', borderLeft:'1px solid var(--color-border)', padding:16, flexShrink:0 }}>
            ...
            <div style={{ marginBottom:12 }}>
              <label style={{ display:'block', fontSize:12, color:'var(--color-text-secondary)', marginBottom:5 }}>Rotation</label>
              <input type="range" min={0} max={360} step={15} value={(map.doors||[]).find(d=>d.id===selectedDoor)?.rotation||0} onChange={e=>{setMap(m=>({...m,doors:(m.doors||[]).map(d=>d.id===selectedDoor?{...d,rotation:+e.target.value}:d)}));onSaveMap(map);}} style={{ width:'100%' }} />
              <div style={{ fontSize:11, color:'var(--color-text-tertiary)', textAlign:'center' }}>{(map.doors||[]).find(d=>d.id===selectedDoor)?.rotation||0}°</div>
            </div>
            <div style={{ marginBottom:12 }}>
              <label style={{ display:'block', fontSize:12, color:'var(--color-text-secondary)', marginBottom:5 }}>Size</label>
              <input type="range" min={24} max={80} value={(map.doors||[]).find(d=>d.id===selectedDoor)?.size||40} onChange={e=>{setMap(m=>({...m,doors:(m.doors||[]).map(d=>d.id===selectedDoor?{...d,size:+e.target.value}:d)}));onSaveMap(map);}} style={{ width:'100%' }} />
            </div>
            <button onClick={deleteSelected} style={{ ...btnS, width:'100%', fontSize:12, color:'var(--color-danger-text)', borderColor:'var(--color-danger-border)' }}>🗑 Delete</button>
          </div>
        )}
        {!editing&&selectedLocObj&&(
          <div style={{ width:320, background:'var(--color-bg)', borderLeft:'1px solid var(--color-border)', overflow:'auto', padding:20, flexShrink:0 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <div>
                <div style={{ fontSize:17, fontWeight:700 }}>{selectedLocObj.icon} {selectedLocObj.name}</div>
                <div style={{ fontSize:12, color:'var(--color-text-secondary)', marginTop:2 }}>{selectedStock.length} units</div>
              </div>
            ...
            {Object.values(selectedGrouped).map(({ item, entries }) => {
              const worst=worstStatus(entries); const ws=SS[worst]; const hasIssue=worst==='expired'||worst==='soon';
              return <div key={item.id} onClick={()=>navigate('drugdetail',{libraryId:item.id})} style={{ marginBottom:10, padding:'10px 12px', background:'var(--color-bg-secondary)', border:hasIssue?`1.5px solid ${ws.border}`:'1px solid var(--color-border)', borderRadius:'var(--radius-md)', cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                  <div style={{ fontWeight:600, fontSize:13 }}>{item.name}</div>
                  <Badge type={worst} label={`${entries.length} units`} />
                </div>
                {[...entries].sort((a,b)=>{const da=parseExp(a.expiration),db=parseExp(b.expiration);if(!da&&!db)return 0;if(!da)return 1;if(!db)return -1;return da-db;}).slice(0,3).map(e=>{
                  const st=getStatus(e.expiration);
                  return <div key={e.id} style={{ fontSize:12, display:'flex', justifyContent:'space-between', padding:'3px 0', borderTop:'1px solid var(--color-border)' }}>
                    <span style={{ fontFamily:'var(--font-mono)', fontWeight:600 }}>{e.expiration==='NA'?'N/A':e.expiration||'—'}</span>
                    <Badge type={st.type} label={st.label} />
                  </div>;
                })}
...
function InventoryView({ library, stock, locations, categories, navigate, onSaveCategories, onSaveStock }) {
  ...
  const items = library
    ...
    .map(item => {
      ...
      return { item, entries, worst };
    })
    .filter(({ entries }) => entries.length > 0)
    .filter(({ worst }) => {
      if (filter === 'issues') return worst === 'expired' || worst === 'soon';
      if (filter === 'watch')  return worst === 'watch';
      if (filter === 'good')   return worst === 'good' || worst === 'none';
      return true;
    })
    .sort((a, b) => ['expired','soon','watch','none','good'].indexOf(a.worst) - ['expired','soon','watch','none','good'].indexOf(b.worst));

  function getAvailableUnits(libItem, locId) {
    return active.filter(s => s.libraryId === libItem?.id && s.locationId === locId)
      .sort((a, b) => {
        if (a.expiration==='NA'&&b.expiration==='NA') return 0;
  ...
  function confirmMove() {
    const ids = unitsToMove.map(s => s.id);
    onSaveStock(stock.map(s => ids.includes(s.id) ? { ...s, locationId: moveTo } : s));
    closeMove();
  }
    ...
    <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        {items.map(({ item, entries, worst }) => {
          ...
          const cat=categories.find(c=>c.id===item.category);
          const byLocation={};
          entries.forEach(s=>{ const loc=locations.find(l=>l.id===s.locationId); const key=loc?.name||'Unknown'; byLocation[key]=(byLocation[key]||0)+1; });
      ...
      {movingItem&&(
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', display:'flex', alignItems:'flex-end', zIndex:200 }}>
          <div style={{ width:'100%', maxWidth:680, margin:'0 auto', background:'var(--color-bg)', borderRadius:'20px 20px 0 0', padding:'24px 20px 36px', maxHeight:'85vh', overflow:'auto' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
              <div>
                <div style={{ fontSize:16, fontWeight:700 }}>Move {movingItem.name}</div>
                <div style={{ fontSize:12, color:'var(--color-text-secondary)', marginTop:2 }}>FIFO — oldest expiration moves first</div>
              </div>
              ...
              <>
                <div style={{ background:'var(--color-bg-secondary)', borderRadius:'var(--radius-lg)', padding:'16px', marginBottom:20 }}>
                  ...
                  <div style={{ borderTop:'1px solid var(--color-border)', paddingTop:12 }}>
                    ...
                    {unitsToMove.map((entry,i)=>{ const st=getStatus(entry.expiration); const es=SS[st.type];
                      return <div key={entry.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 10px', background:'var(--color-bg)', border:`1px solid ${es.border}`, borderRadius:8, marginBottom:6 }}>
                        <span style={{ fontSize:14, fontWeight:700, fontFamily:'var(--font-mono)' }}>Unit {i+1} — {entry.expiration==='NA'?'N/A':entry.expiration||'No date'}</span>
                        <Badge type={st.type} label={st.label} />
                      </div>;
                    })}
                  </div>
          ...
          <div style={{ marginTop:12 }}>
            ...
            {showPulled&&pulled.sort((a,b)=>new Date(b.pulledAt||0)-new Date(a.pulledAt||0)).map(entry=>(
              <div key={entry.id} style={{ display:'flex', alignItems:'center', padding:'9px 14px', background:'var(--color-bg-secondary)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-md)', marginBottom:6, opacity:0.55 }}>
                <div style={{ flex:1 }}>
                  <span style={{ fontSize:13, fontWeight:600, textDecoration:'line-through', fontFamily:'var(--font-mono)' }}>{entry.expiration}</span>
                  <span style={{ fontSize:11, color:'var(--color-text-tertiary)', marginLeft:8 }}>Pulled {entry.pulledAt?new Date(entry.pulledAt).toLocaleDateString():''}</span>
                </div>
...
function AddStockView({ libraryId, locationId, library, stock, locations, navigate, onSaveStock }) {
  ...
  function handleSave() {
    ...
    setSaving(false);
    if (locationId) navigate('locationdetail',{locationId});
    else if (libraryId) navigate('drugdetail',{libraryId});
    else navigate('inventory');
  }

  if (step==='selectitem') return (
    <div>
      <TopBar title="Add Stock" onBack={()=>navigate(-1)} />
      <div style={{ padding:'0 20px' }}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search library..." style={{ marginBottom:12 }} />
        {library.filter(i=>i.status!=='inactive').filter(i=>!q||i.name.toLowerCase().includes(q)).map(item=>(
          <div key={item.id} onClick={()=>{setSelectedItem(item);setStep('location');}} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 14px', background:'var(--color-bg)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-lg)', marginBottom:8, cursor:'pointer' }}>
  ...
  if (step==='location') return (
    <div>
      <TopBar title={selectedItem?.name} onBack={()=>setStep('selectitem')} />
      <div style={{ padding:'0 20px' }}>
        <div style={{ fontSize:13, color:'var(--color-text-secondary)', marginBottom:14 }}>Where is this stock going?</div>
        {locations.map(loc=>(
          <div key={loc.id} onClick={()=>{setSelectedLocation(loc.id);setStep('count');}} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', background:selectedLocation===loc.id?'var(--color-bg-secondary)':'var(--color-bg)', border:selectedLocation===loc.id?'2px solid var(--color-border-strong)':'1px solid var(--color-border)', borderRadius:'var(--radius-lg)', marginBottom:8, cursor:'pointer' }}>
            <span style={{ fontSize:20 }}>{loc.icon}</span><span style={{ fontWeight:600, fontSize:14 }}>{loc.name}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  if (step==='count') return (
    <div>
      <TopBar title={selectedItem?.name} onBack={()=>setStep('location')} />
      <div style={{ padding:'0 20px' }}>
  ...
  if (step==='expirations') return (
    <div>
      <TopBar title={selectedItem?.name} onBack={()=>setStep('count')} />
      <div style={{ padding:'0 20px' }}>
        <div style={{ background:'var(--color-bg-secondary)', borderRadius:'var(--radius-md)', padding:'10px 14px', marginBottom:16, fontSize:13, color:'var(--color-text-secondary)', textAlign:'center' }}>{expirations.length} unit{expirations.length!==1?'s':''} → {locations.find(l=>l.id===selectedLocation)?.name}<br/>Enter expiration — tap N/A if none</div>
...
function LibraryView({ library, stock, categories, navigate }) {
  ...
  const visible = library
    ...
    .sort((a, b) => {
      const order=['expired','soon','watch','none','good'];
      const wa=worstStatus(active.filter(s=>s.libraryId===a.id));
      const wb=worstStatus(active.filter(s=>s.libraryId===b.id));
      if (wa!==wb) return order.indexOf(wa)-order.indexOf(wb);
    ...
    <div style={{ paddingBottom:20 }}>
      ...
      <div style={{ padding:'0 20px' }}>
        ...
        {visible.map(item => {
          ...
          const cat=categories.find(c=>c.id===item.category); const par=item.stationPar||0;
          const usable=ds.filter(s=>getStatus(s.expiration).type!=='expired').length; const belowPar=par>0&&usable<par;
          return (
...
function AddItemView({ libraryId, scanData, capturedPhoto, library, stock, locations, categories, navigate, onSaveLibrary, onSaveStock }) {
  ...
  async function handleSave() {
    ...
    const updated = { id, ...form, profilePhoto:photo, stationPar:parseInt(form.stationPar)||0, addedAt:existing?.addedAt||new Date().toISOString() };
    await onSaveLibrary(existing ? library.map(d=>d.id===id?updated:d) : [updated,...library]);
    setSaving(false);
    navigate('library');
  }
  ...
  async function handlePhotoChange(e) {
    const file=e.target.files?.[0]; if(!file) return;
    const reader=new FileReader();
    reader.onload=async ev=>setPhoto(await resizeImage(ev.target.result.split(',')[1],400));
    reader.readAsDataURL(file);
  }
            ...
            {!scanning?(
              <SmartCamera
                onBarcodeResult={async barcode => {
                  ...
                  try {
                    const data = await api.ndcLookup(barcode);
                    if (data.found) {
                      setForm(f=>({...f, name:data.name||f.name, notes:[data.route,data.dosageForm,data.packager].filter(Boolean).join(', ')||f.notes}));
                      setScanned(true);
                    } else { setScanError('Barcode not in FDA database — try label scan or fill in manually'); }
                  } catch { setScanError('Lookup failed'); }
                  setScanning(false);
                }}
                onPhotoCapture={async b64 => {
                  setScanning(true); setScanError(null);
                  try {
                    const result = await api.scan([b64]);
                    ...
                    if (!photo) setPhoto(await resizeImage(b64,400));
                    setScanned(true);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handler = () => {
      setNavStack(prev => {
        if (prev.length>1) { window.history.pushState({idx:prev.length-1},''); return prev.slice(0,-1); }
        window.history.pushState({idx:0},''); return prev;
      });
    };
  ...
  function navigate(view, params = {}) {
    if (view===-1) { setNavStack(prev=>prev.length>1?prev.slice(0,-1):prev); return; }
    window.history.pushState({idx:navStack.length},'');
  ...
  useEffect(() => {
    Promise.all([api.getLibrary(),api.getStock(),api.getLocations(),api.getCategories(),api.getTemplates(),api.getMap()])
      .then(([lib,stk,locs,cats,tmpls,map])=>{
        setLibrary(lib||[]);
        setStock(stk||[]);
        setLocations(locs?.length?locs:DEFAULT_LOCATIONS);
        setCategories(cats?.length?cats:DEFAULT_CATEGORIES);
        setTemplates(tmpls||[]);
        setMapData(map||{rooms:[],pins:[],lines:[],doors:[],bgImage:null});
        setLoading(false);
      })
      .catch(()=>{ setLocations(DEFAULT_LOCATIONS); setCategories(DEFAULT_CATEGORIES); setLoading(false); });
  }, []);

  async function persist(setter, data, saveFn) {
    setter(data); setSaveStatus('Saving...');
    try { await saveFn(data); setSaveStatus('Saved ✓'); setTimeout(()=>setSaveStatus(''),2000); }
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100vh', flexDirection:'column', gap:16 }}>
      <Spinner/>
      <div style={{ fontSize:14, color:'var(--color-text-secondary)' }}>Loading EMS Inventory...</div>
    </div>
  );
